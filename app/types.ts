...
export const groupBy = (key: keyof Transaction, transactions: Transaction[]) => {
  return transactions.reduce((acc: { [key: string]: Transaction[] }, transaction) => {
    const groupKey = transaction[key]?.toString() ?? 'unknown';
...
export const groupedTransactions = groupBy('date', transactions);
